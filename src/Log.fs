...
// helper function so that i can pass a list of arguments to write functions.
...
let myPrint fmt = List.rev >> myPrintHelper<string> fmt

type ILogFormat = 
    abstract writeInformation : string -> string list -> Log<string list, unit>
    abstract writeWarning : string -> string list -> Log<string list, unit>
    abstract writeError : string -> string list -> Log<string list, unit>

type HtmlLogger() = 
    interface ILogFormat with
        member x.writeInformation format args = logMessage ("<li class='log-information'>" + myPrint format args + "</li>")
        member x.writeWarning format args = logMessage ("<li class='log-warning'>" + myPrint format args + "</li>")
        member x.writeError format args = logMessage ("<li class='log-error'>" + myPrint format args + "</li>")

type StringLogger() =
    interface ILogFormat with
        member x.writeInformation format args = logMessage("Information... " + myPrint format args)
        member x.writeWarning format args = logMessage("Warning... " + myPrint format args)
        member x.writeError format args = logMessage("Error... " + myPrint format args)

let printLog log =
    let (result, logs) = log
    logs |> Seq.iter(fun l -> printfn "%s" l)
    printfn "Upgrade success = %A." result
